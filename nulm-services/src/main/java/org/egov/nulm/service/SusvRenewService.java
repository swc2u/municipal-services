...
public class SusvRenewService {
	...
	@Autowired
	public SusvRenewService(SusvRenewRepository repository, ObjectMapper objectMapper,
			IdGenRepository idgenrepository, NULMConfiguration config, AuditDetailsUtil auditDetailsUtil,WorkFlowRepository workFlowRepository) {
		...
		try {
			...
			susvRenewapplication.setAuditDetails(
			   ...
			   // idgen service call to genrate event id
				IdGenerationResponse id = idgenrepository.getId(request.getRequestInfo(), susvRenewapplication.getTenantId(),
						config.getSusvRenewApplicationNumberIdgenName(), config.getSusvRenewApplicationNumberIdgenFormat(), 1);
	...
	public ResponseEntity<ResponseInfoWrapper> updateSusvApplication(NulmSusvRenewRequest request) {
		try {
			SusvRenewApplication susvRenewApplication = objectMapper.convertValue(request.getNulmSusvRenewRequest(),
					SusvRenewApplication.class);
			susvRenewApplication.setIsActive(true);
			susvRenewApplication.setAuditDetails(auditDetailsUtil.getAuditDetails(request.getRequestInfo(), CommonConstants.ACTION_UPDATE));
	...
	public ResponseEntity<ResponseInfoWrapper> getSuhRenewApplication(NulmSusvRenewRequest request) {
		try {
			...
			List<SusvRenewApplication> SuhApplicationresult = repository.getSusvRenewApplication(susvRenewapplication, request.getRequestInfo().getUserInfo().getType(), request.getRequestInfo().getUserInfo().getId());
			return new ResponseEntity<>(ResponseInfoWrapper.builder()
					.responseInfo(ResponseInfo.builder().status(CommonConstants.SUCCESS).build())
