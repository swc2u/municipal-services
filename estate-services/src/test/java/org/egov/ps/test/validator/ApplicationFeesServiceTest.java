...
public class ApplicationFeesServiceTest {
	...
	public void testSimpleApplicationFeesValidation() throws JSONException {
		...
		when(mdmsService.getApplicationFees(eq(SampleApplicationType), any(RequestInfo.class), any(String.class)))
				.thenReturn(results);
		...
		assertTrue(new BigDecimal(6000.0).compareTo(
				applicationEnrichmentService.fetchEstimateAmountFromMDMSJson(results, categoryMatchApplication)) == 0);
		assertTrue(new BigDecimal(5000.0).compareTo(applicationEnrichmentService
				.fetchEstimateAmountFromMDMSJson(results, categorySubCateMatchApplication)) == 0);
		assertTrue(new BigDecimal(100.0).compareTo(
				applicationEnrichmentService.fetchEstimateAmountFromMDMSJson(results, noneMatchApplcaition)) == 0);
	}
