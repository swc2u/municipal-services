		...
		try {
			...
			for (PaymentDetail paymentDetail : paymentRequest.getPayment().getPaymentDetails()) {
				if (paymentDetail.getBusinessService().equalsIgnoreCase(WCConstants.WATER_SERVICE_BUSINESS_ID)) {
					BillGeneration bill = BillGeneration.builder().paymentStatus(WCConstants.STATUS_PAID).status(WCConstants.STATUS_PAID).billGenerationId(paymentDetail.getBill().getConsumerCode()).receiptDate(paymentDetail.getReceiptDate()).
			...
			for (PaymentDetail paymentDetail : paymentRequest.getPayment().getPaymentDetails()) {
				...
				if (paymentDetail.getBusinessService().equalsIgnoreCase(config.getReceiptBusinessservice())) {
					...
					waterConnectionRequest.getWaterConnection().getWaterApplication().setTotalAmountPaid(paymentDetail.getTotalAmountPaid().toString());;
