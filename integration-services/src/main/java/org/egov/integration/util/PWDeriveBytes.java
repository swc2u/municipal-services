...
public class PWDeriveBytes {
    ...
    public PWDeriveBytes(String password, byte[] salt) {
        try {
            this.hash = MessageDigest.getInstance("SHA-1");

            this.hash.update(password.getBytes(StandardCharsets.US_ASCII));
        ...
        try {
            while (generated < requested) {
                ...
                if (outputOffset == 0 && position != 0) {
                    ...
                    hash.update(counter.getBytes(StandardCharsets.US_ASCII));
                    hash.update(firstToLastDigest);
