...
public class PurchaseOrderJdbcRepository extends org.egov.assets.common.JdbcRepository {
	...
	public Pagination<PurchaseOrder> searchByIndentData(PurchaseOrderSearch is, String sort, Integer pagesize,
			Integer offser) {
		String searchQuery = "select * from purchaseorder where purchaseordernumber in (\r\n"
				+ "select purchaseorder FROM indent inner join purchaseorderdetail "
				+ "on indent.indentnumber=purchaseorderdetail.indentnumber :condition" + "  ) :poFilters :orderby   ";

		Map<String, Object> paramValues = new HashMap<>();
		Map<String, Object> poParamValues = new HashMap<>();

		StringBuffer params = new StringBuffer();
		StringBuffer poParams = new StringBuffer();

		if (sort != null && !sort.isEmpty()) {
			validateSortByOrder(sort);
			validateEntityFieldName(sort, PurchaseOrderEntity.class);
		}
		...
		Pagination<PurchaseOrder> page = new Pagination<>();
		if (offser != null) {
			page.setOffset(offser - 1);
		}
		if (pagesize != null) {
			page.setPageSize(pagesize);
		}

		// paramValues.put("indentpurpose", is1.getIndentPurpose());
		// paramValues.put("inventorytype", is1.getInventoryType());
		// paramValues.put("indentcreatedby", is1.getIndentRaisedBy());
		// paramValues.put("fromDate", is.getIndentFromDate());
		// paramValues.put("toDate", is.getIndentToDate());

		if (is.getIndentPurpose() != null) {
			if (params.length() > 0)
				params.append(" and ");
			params.append("indentpurpose =:indentpurpose");
			paramValues.put("indentpurpose", is.getIndentPurpose());
		}

		if (is.getInventoryType() != null) {
			if (params.length() > 0)
				params.append(" and ");
			params.append("inventorytype =:inventorytype");
			paramValues.put("inventorytype", is.getInventoryType());
		}

		if (is.getIndentRaisedBy() != null) {
			if (params.length() > 0)
				params.append(" and ");
			params.append("indentcreatedby =:indentcreatedby");
			paramValues.put("indentcreatedby", is.getIndentRaisedBy());
		}

		if (is.getIndentFromDate() != null) {
			if (params.length() > 0)
				params.append(" and ");
			params.append(
					"TO_DATE(TO_CHAR(TO_TIMESTAMP(createdtime / 1000), 'YYYY-MM-DD'),'YYYY-MM-DD') >= TO_DATE(TO_CHAR(TO_TIMESTAMP(:fromDate / 1000), 'YYYY-MM-DD'),'YYYY-MM-DD')");
			paramValues.put("fromDate", is.getIndentFromDate());
		}

		if (is.getIndentToDate() != null) {
			if (params.length() > 0)
				params.append(" and ");
			params.append(
					"TO_DATE(TO_CHAR(TO_TIMESTAMP(createdtime / 1000), 'YYYY-MM-DD'),'YYYY-MM-DD') <= TO_DATE(TO_CHAR(TO_TIMESTAMP(:toDate / 1000), 'YYYY-MM-DD'),'YYYY-MM-DD')");
			paramValues.put("toDate", is.getIndentToDate());
		}

		if (is.getStore() != null) {
			if (poParams.length() > 0)
				poParams.append(" and ");
			poParams.append("store =:store ");
			poParamValues.put("store", is.getStore());
		}

		if (is.getPurchaseOrderNumber() != null) {
			if (poParams.length() > 0)
				poParams.append(" and ");
			poParams.append(" purchaseordernumber =:purchaseordernumber");
			poParamValues.put("purchaseordernumber", is.getPurchaseOrderNumber());
		}
		if (is.getSupplier() != null) {
			if (poParams.length() > 0)
				poParams.append(" and ");
			poParams.append("supplier =:supplier");
			poParamValues.put("supplier", is.getSupplier());
		}
		if (is.getRateType() != null) {
			if (poParams.length() > 0)
				poParams.append(" and ");
			poParams.append("ratetype =:rateType");
			poParamValues.put("rateType", is.getRateType());
		}
		...
		if (poParams.length() > 0) {
			searchQuery = searchQuery.replace(":poFilters", " and " + poParams.toString());
		} else
			searchQuery = searchQuery.replace(":poFilters", "");

		if (poParamValues != null || !poParamValues.isEmpty())
			poParamValues.entrySet().stream().forEach(e -> paramValues.put(e.getKey(), e.getValue()));
