...
public class PriceListService extends DomainService {
	...
	public PriceListResponse save(PriceListRequest priceListRequest, String tenantId) {
		...
		try {
			...
			List<String> priceListIdList = priceListJdbcRepository.getSequence(PriceList.class.getSimpleName(),
					priceListRequest.getPriceLists().size());
				...
				priceList.getPriceListDetails().forEach(priceListDetail -> {
					...
					if (priceListDetail.getUom().getCode() != null) {
						if (priceListDetail.getQuantity() != null)
							priceListDetail.setQuantity((priceListDetail.getUom().getConversionFactor()).doubleValue()
									* priceListDetail.getQuantity());
					}
				});
			});
			...
			PriceListResponse response = PriceListResponse.builder().priceLists(priceListRequest.getPriceLists())
					.responseInfo(getResponseInfo(priceListRequest.getRequestInfo())).build();
	...
	public PriceListResponse update(PriceListRequest priceListRequest, String tenantId) {

		try {

			// TODO: Do not allow them to modify supplier,rate type, agreement start and end
			// date in modify level.
			...
			priceListRequest.getPriceLists().stream().forEach(priceList -> {
				...
				for (PriceListDetails priceListDetail : priceList.getPriceListDetails()) {
					if (priceListDetail.getId() == null) {
						priceListDetail.setId(priceListDetailsJdbcRepository
								.getSequence(PriceListDetails.class.getSimpleName(), 1).get(0));
						priceListDetail.setAuditDetails(mapAuditDetails(priceListRequest.getRequestInfo()));
					}
					if (priceListDetail.getTenantId() == null) {
						priceListDetail.setTenantId(tenantId);
				...
				priceListDetailsJdbcRepository.markDeleted(ids, tenantId, "pricelistdetails", "pricelist",
						priceListRequest.getPriceLists().get(0).getId());
			...
			response.setResponseInfo(getResponseInfo(priceListRequest.getRequestInfo()));
	...
	public PriceListResponse search(PriceListSearchRequest priceListSearchRequest, RequestInfo requestInfo) {

		Pagination<PriceList> searchPriceLists = priceListRepository.search(priceListSearchRequest);

		PriceListResponse response = new PriceListResponse();
		response.setPriceLists(
				searchPriceLists.getPagedData().size() > 0 ? searchPriceLists.getPagedData() : Collections.emptyList());
		return response;
	}

	public PriceListResponse searchPriceList(PriceListSearchRequest priceListSearchRequest, RequestInfo requestInfo) {
	...
	private void validate(List<PriceList> priceLists, String method, String tenantId,
		...
		try {
			...
			case Constants.ACTION_CREATE: {
				...
				for (PriceList pl : priceLists) {
					...
					if (!priceListJdbcRepository.uniqueCheck("agreementNumber", new PriceListEntity().toEntity(pl))) {
			...
			for (PriceList pl : priceLists) {
				...
				for (PriceListDetails pld : pl.getPriceListDetails()) {
					...
					if (pld.getRatePerUnit() <= 0) {
						errors.addDataError(ErrorCode.UNIT_PRICE_GT_ZERO.getCode(),
								null + " at serial no." + (pl.getPriceListDetails().indexOf(pld) + 1));
					}
				...
				if (Long.valueOf(pl.getAgreementDate()) > currentMilllis) {
					errors.addDataError(ErrorCode.DATE_LE_CURRENTDATE.getCode(), "agreementDate",
							convertEpochtoDate(pl.getAgreementDate()));
				}

				if (Long.valueOf(pl.getRateContractDate()) > currentMilllis) {
					errors.addDataError(ErrorCode.DATE_LE_CURRENTDATE.getCode(), "rateContractDate",
							convertEpochtoDate(pl.getRateContractDate()));
				}
				...
				if (Long.valueOf(pl.getAgreementEndDate()) < Long.valueOf(pl.getAgreementStartDate())) {
					errors.addDataError(ErrorCode.DATE1_GE_DATE2.getCode(), "agreementEndDate", "agreementStartDate",
							convertEpochtoDate(pl.getAgreementEndDate()),
							convertEpochtoDate(pl.getAgreementStartDate()));

				}
				// No clear input in hld.
				/*
				 * if (Long.valueOf(pl.getAgreementStartDate()) <
				 * Long.valueOf(pl.getAgreementDate())) {
				 * errors.addDataError(ErrorCode.DATE1_GE_DATE2.getCode(), "agreementStartDate",
				 * "agreementDate", convertEpochtoDate(pl.getAgreementStartDate()),
				 * convertEpochtoDate(pl.getAgreementDate()));
				 * 
				 * }
				 */
		...
		plr.setPriceLists(Arrays.asList(PriceList.builder()
				.priceListDetails(Arrays.asList(PriceListDetails
						.builder().tenderUsedQuantity(purchaseOrderJdbcRepository
								.getTenderUsedQty(material, priceListId).divide(uomFetch.getConversionFactor()))
						.build()))
				.build()));
		return plr;
	}
