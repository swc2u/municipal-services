...
public abstract class JdbcRepository {
	...
	public Map<String, Object> paramValues(Object ob, List<String> fields) {
		...
		for (String s : fields) {
			...
			/*
			 * try { f = ob.getClass().getSuperclass().getDeclaredField(s); } catch
			 * (NoSuchFieldException e1) { System.out.println(
			 * "Unable to find the field in this class and its super class for field" + s);
			 * } }
			 */
			try {
	...
	@Transactional
	public void updateQuantity(String tableName, String id, String tenantId, HashMap<String, BigDecimal> columns,
			String operation) {
		String updateQuery = null;

		if (operation.equalsIgnoreCase("ADD")) {
			updateQuery = "update " + tableName + " set " + addColumn(columns, "+") + " where id = '" + id
					+ "' and tenantid = " + tenantId;
		}

		if (operation.equalsIgnoreCase("SUBTRACT")) {
			updateQuery = "update " + tableName + " set " + addColumn(columns, "-") + " where id = '"
					+ "' and tenantid = " + tenantId;
		}
	...
	public Boolean uniqueCheck(String firstFieldName, String secondFieldName, Object ob) {
		...
		StringBuffer uniqueQuery = new StringBuffer("select count(*) as count from " + table + " where "
				+ firstFieldName + "=:firstFieldValue" + " and " + secondFieldName + "=:secondFieldValue");
	 ...
	 * @param parentPkValue
	 */
	public void markDeleted(List<String> ids, String tenantId, String tableName, String parentPkFieldName,
	...
	public void markDeletedPKeyIn(List<String> ids, String tenantId, String tableName, String parentPkFieldName,
			List<String> parentPkValue) {
		Map<String, Object> paramValues = new HashMap<>();
		String query = "update " + tableName + " set deleted=true where id not in (:ids) and tenantId=:tenantId and "
				+ parentPkFieldName + " in (:parentPkValue)";
		paramValues.put("ids", ids);
		paramValues.put("tenantId", tenantId);
		paramValues.put("parentPkValue", parentPkValue);
	...
	public void delete(Object entity, String reason) {
		...
		backupQuery.append("insert into " + backupTable + " select '1',:tablename,id,tenantid,:reason,row_to_json("
				+ table + "),now() " + " from " + table + " where tenantid=:tenantId and id=:id ");
		System.out.println("query.............." + backupQuery);
		namedParameterJdbcTemplate.batchUpdate(backupQuery.toString(),
				batchValues.toArray(new Map[batchValues.size()]));
		...
		namedParameterJdbcTemplate.batchUpdate(deleteQuery.toString(),
				batchValues.toArray(new Map[batchValues.size()]));
	...
	public Object findById(Object entity, String entityName) {
		...
		List<Object> indents = namedParameterJdbcTemplate.query(getByIdQuery.get(entityName), paramValues,
